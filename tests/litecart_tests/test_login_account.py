...
def test_login_and_edit_account(browser, test_case_1_data, cursor):
    with allure.step("Open main page"):
        main_page = MainPage(browser)
        main_page.open_main_page()
    with allure.step("Check the user can login"):
        main_page.login(test_case_1_data)
    ...
    with allure.step("Check edit account page is displayed"):
        ...
        account_page.edit_account_page(test_case_1_data)
    with allure.step("Check the user can change name"):
        account_page.change_username(test_case_1_data)

    with allure.step("Check the username was changed"):
        changes_in_db(cursor, test_case_1_data)
